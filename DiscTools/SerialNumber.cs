...
namespace DiscTools
{
    public class SerialNumber
    {
        public static string GetSaturnSerial(string cuePath)
        {
            ...
            const int lba = 0;
            var disc = Disc.LoadAutomagic(cuePath);
            ...
            */
            ...
            var di = new DiscIdentifier(disc);

            // start at lba 0 (saturn header information is usually there)
            var data = di.GetPSXSerialNumber(lba);
            ...
            {
                var blockSize = 32;
                var data32 = data.ToList().Skip(i * (blockSize)).Take(blockSize).ToArray();
        ...
        public static string GetPSXSerial(string cuePath)
        {
            ...
            var data = di.GetPSXSerialNumber(lba);
            // take first 32 bytes
            var data32 = data.ToList().Take(46).ToArray();
            ...
            var arr = sS.Split(new string[] { "cdrom:" }, StringSplitOptions.None);
            var arr2 = arr[1].Split(new string[] { ";1" }, StringSplitOptions.None);
            var serial = arr2[0].Replace("_", "-").Replace(".", "");
            if (serial.Contains("\\"))
